	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
		<view class="content">
			<view :style='{"alignContent":"flex-start","minHeight":"100vh","padding":"0 24rpx 128rpx","borderColor":"#c0c0c0","alignItems":"flex-start","flexWrap":"wrap","background":"#fff","borderWidth":"2rpx 0 0 0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}'>
					...
					<view class="list" :style='{"width":"100%","padding":"20rpx 20rpx 2rpx","margin":"0px 0 0","background":"linear-gradient(135.00deg, rgb(238, 230, 164) 0.464%,rgb(215, 237, 239) 100%)","height":"auto"}'>
						<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"padding":"12rpx","margin":"0 0 40rpx","backgroundColor":"#fff","color":"#999","flexWrap":"wrap","display":"flex","width":"100%","fontSize":"28rpx","height":"auto"}' v-for="(product,index) in list" :key="index">
							...
							<view class="listmr" :style='{"alignContent":"center","padding":"0 0 0 20rpx","margin":"0","alignItems":"center","flexWrap":"wrap","display":"flex","width":"calc(100% - 200rpx)","height":"auto"}'>
								...
								<view class="col3 f30 elip mb15" :style='{"padding":"0","margin":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#224366","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'>陪玩态度:{{product.peiwantaidu}}</view>
								<view class="col3 f30 elip mb15" :style='{"padding":"0","margin":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#224366","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'>用户姓名:{{product.yonghuxingming}}</view>
							...
							<view :style='{"width":"100%","padding":"8rpx 20rpx","margin":"20rpx 0 0 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"border":"2rpx solid #174add","padding":"10rpx","background":"#d3e5ff","display":"flex"}' v-if="(userid && isAuth('peiwanpingjia','修改')) || (!userid && isAuthFront('peiwanpingjia','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"border":"2rpx solid #9a9a9a","padding":"10rpx","background":"#ecebeb","display":"flex"}' v-if="(userid && isAuth('peiwanpingjia','删除')) || (!userid && isAuthFront('peiwanpingjia','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"8rpx 20rpx","margin":"20rpx 0 0 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"border":"2rpx solid #174add","padding":"10rpx","background":"#d3e5ff","display":"flex"}' v-if="(userid && isAuth('peiwanpingjia','修改')) || (!userid && isAuthFront('peiwanpingjia','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#174add","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#174add","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"border":"2rpx solid #9a9a9a","padding":"10rpx","background":"#ecebeb","display":"flex"}' v-if="(userid && isAuth('peiwanpingjia','删除')) || (!userid && isAuthFront('peiwanpingjia','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>删除</text>
								</view>
							</view>
				...
				<button :style='{"border":"0","color":"rgb(255, 255, 255)","bottom":"128rpx","outline":"none","borderRadius":"100%","left":"40rpx","background":"#EF7061","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('peiwanpingjia','新增')" class="add-btn" @click="onAddTap()">新增</button>
				<button :style='{"border":"0","color":"rgb(255, 255, 255)","bottom":"128rpx","outline":"none","borderRadius":"100%","left":"40rpx","background":"#EF7061","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="!userid && isAuthFront('peiwanpingjia','新增')" class="add-btn" @click="onAddTap()">新增</button>
				...
				<view class="screenBox" :class="screenBoxShow?'screenBoxActive':''">
					<view :style='{"width":"100%","padding":"20rpx 0 20rpx 0"}'>
						<view :style='{"width":"100%","padding":"0 0 20rpx 20rpx","fontSize":"24rpx"}'>游戏能力</view>
						<view :style='{"width":"100%","flexWrap":"wrap","justifyContent":"space-around","display":"flex"}'>
							<view @click="youxinengliChange(item)" v-for="(item,index) in youxinengliOptions" :key="index" class="screenTab" :class="searchForm.youxinengli==item?'screenTabActive':''">{{item}}</view>
						</view>
					</view>
					<view :style='{"width":"100%","padding":"20rpx 0 20rpx 0","alignItems":"center","justifyContent":"space-around","display":"flex"}'>
						<view :style='{"width":"auto","padding":"0 20rpx 0 20rpx","fontSize":"24rpx","whiteSpace":"nowrap"}'>用户账号</view>
						<input :style='{"border":"0","padding":"0 20rpx 0 20rpx","margin":"0 16rpx 0 0","borderRadius":"8rpx","background":"#f3f5f9","flex":"1","width":"75%","fontSize":"28rpx","height":"68rpx"}' placeholder="请输入用户账号" v-model="searchForm.yonghuzhanghao">
					</view>
...
<script>
	export default {
		...
		async onLoad(options) {
			...
			// if (this.mescroll) this.mescroll.resetUpScroll()
			let res = {};
			this.youxinengliOptions = '1星,2星,3星,4星,5星'.split(',')
		},
		components: {
		},
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.youxinengli){
					params['youxinengli'] = '%' + this.searchForm.youxinengli + '%'
				}
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`peiwanpingjia`, params);
                } else {
                    res = await this.$api.list(`peiwanpingjia`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('peiwanpingjia', JSON.stringify([id]));
			...
			async search(){
				...
				if(this.searchForm.youxinengli){
					searchForm['youxinengli'] = '%' + this.searchForm.youxinengli + '%'
				}
				if(this.searchForm.yonghuzhanghao){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`peiwanpingjia`, searchForm);
                } else {
                    res = await this.$api.list(`peiwanpingjia`, searchForm);
                }
				// 如果是第一页数据置空
