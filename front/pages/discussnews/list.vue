			...
			<view :style='{"alignContent":"flex-start","minHeight":"100vh","padding":"0 24rpx 128rpx","borderColor":"#c0c0c0","alignItems":"flex-start","flexWrap":"wrap","background":"#fff","borderWidth":"2rpx 0 0 0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}'>
				<view class="cu-bar bg-white search" :style='{"width":"calc(100% - 90rpx)","padding":"20rpx 0","background":"none","display":"flex","height":"auto"}'>
					<view :style='{"border":"2rpx solid #b0cb5e","margin":"0 12rpx 0 0","borderRadius":"60rpx","flex":"1","background":"#f8fff7","position":"relative","height":"88rpx"}' class="search-form round">
						...
						<input placeholder-style="color: #333" :style='{"border":"0","padding":"12rpx 24rpx 12rpx 24rpx","color":"#333","borderRadius":"60rpx","background":"none","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.nickname" type="text" placeholder="用户名" ></input>
					</view>
					...
					<view class="list" :style='{"width":"100%","padding":"20rpx 20rpx 2rpx","margin":"0px 0 0","background":"linear-gradient(135.00deg, rgb(238, 230, 164) 0.464%,rgb(215, 237, 239) 100%)","height":"auto"}'>
						<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"padding":"12rpx","margin":"0 0 40rpx","backgroundColor":"#fff","color":"#999","flexWrap":"wrap","display":"flex","width":"100%","fontSize":"28rpx","height":"auto"}' v-for="(product,index) in list" :key="index">
							...
							<view :style='{"width":"100%","padding":"8rpx 20rpx","margin":"20rpx 0 0 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"border":"2rpx solid #174add","padding":"10rpx","background":"#d3e5ff","display":"flex"}' v-if="(userid && isAuth('discussnews','修改')) || (!userid && isAuthFront('discussnews','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"border":"2rpx solid #9a9a9a","padding":"10rpx","background":"#ecebeb","display":"flex"}' v-if="(userid && isAuth('discussnews','删除')) || (!userid && isAuthFront('discussnews','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"8rpx 20rpx","margin":"20rpx 0 0 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"border":"2rpx solid #174add","padding":"10rpx","background":"#d3e5ff","display":"flex"}' v-if="(userid && isAuth('discussnews','修改')) || (!userid && isAuthFront('discussnews','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#174add","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#174add","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"border":"2rpx solid #9a9a9a","padding":"10rpx","background":"#ecebeb","display":"flex"}' v-if="(userid && isAuth('discussnews','删除')) || (!userid && isAuthFront('discussnews','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>删除</text>
								</view>
							</view>
				...
				<button :style='{"border":"0","color":"rgb(255, 255, 255)","bottom":"128rpx","outline":"none","borderRadius":"100%","left":"40rpx","background":"#EF7061","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('discussnews','新增')" class="add-btn" @click="onAddTap()">新增</button>
				<button :style='{"border":"0","color":"rgb(255, 255, 255)","bottom":"128rpx","outline":"none","borderRadius":"100%","left":"40rpx","background":"#EF7061","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="!userid && isAuthFront('discussnews','新增')" class="add-btn" @click="onAddTap()">新增</button>
				...
				<view class="screenBox" :class="screenBoxShow?'screenBoxActive':''">
					<view :style='{"width":"100%","padding":"20rpx 0 20rpx 0","alignItems":"center","justifyContent":"space-around","display":"flex"}'>
						...
						<input :style='{"border":"0","padding":"0 20rpx 0 20rpx","margin":"0 16rpx 0 0","borderRadius":"8rpx","background":"#f3f5f9","flex":"1","width":"75%","fontSize":"28rpx","height":"68rpx"}' placeholder="请输入评论内容" v-model="searchForm.content">
					</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`discussnews`, params);
                } else {
                    res = await this.$api.list(`discussnews`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('discussnews', JSON.stringify([id]));
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`discussnews`, searchForm);
                } else {
                    res = await this.$api.list(`discussnews`, searchForm);
                }
				// 如果是第一页数据置空
